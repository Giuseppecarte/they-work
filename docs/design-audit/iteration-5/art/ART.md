...
- Every 125 ms sample of a complete lower-aisle sequence was checked against the nameplate rectangles returned by Studio. None overlaps. Integration review additionally caught the host expanding an overview plate from one native row into two when rounding its position; the tower renderer now aligns the label downward to a complete row. The final tower80 capture was inspected: full bodies remain visible above one-row labels. The return ends at the original bounds. All three styles over 100 cycles were additionally checked for facility collisions.
...
Focused verification: [22 artwork tests](tests.log), [frontal-grid check](front-tests.log), [facility collision regression](facilities-tests.log), and [explicit human-wait regression](human-wait-tests.log). The last two regressions were added after the main focused run, adding two tests. Renderer Clippy is recorded in [clippy.log](clippy.log). Workspace, cross-platform and real PTY checks belong to the release verification record.

A final integrated screenshot revealed that an explicit `Wait(HumanInput)` could follow `Editing` without replacing that last activity. The native label correctly said “Question for you”, while the old pose check still drew working. The scene now consults the shared explicit-human-request rule before the activity, after checking failure. Approval/input raises a hand; automatic review, child and process waits do not; failure retains priority. The regression reproduces that exact event-state combination.

Final integration review: [tower at 80×24](../evidence/tower-final/tower-20-80x24-first.png) and [inspector at 192×58](../evidence/tower-final/inspect-team-192x58.png) were inspected after that correction. Noa's raised hand matches the recorded human question; other workers retain their non-human-wait poses. One/two native label rows no longer remove the lower body. The [integrated demo inspector](../ui/inspector-120x36.png) separately shows Jamie C. asking for input while the lead waits for the team without raising a hand. The [design surface](../ui/design-192x58.png) preserves the same facts and uses the lower area for actual floor activity. No additional blocking visual defect was found in these final captures.
