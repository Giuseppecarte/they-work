...
def main():
    ...
    scratch = ROOT / 'docs/design-audit/tmp/iteration-6-control-pty'
    scratch.mkdir(parents=True, exist_ok=True)
    out = ROOT / 'docs/design-audit/iteration-6/evidence' / args.stage
    ...
    try:
        ...
        assert session.text().count('▏') == 1, 'Only the active instruction field should show a caret'
        ...
        result[current] = {'native_thread': 'managed-1', 'native_turn': 'turn-1', 'literal_project_and_prompt': True, 'visible_start_click': True, 'single_active_caret': True}
        ...
        assert 'Instruction accepted' not in session.text(), 'Previous instruction receipt leaked into the approval view'
        ...
        result[current] = {'command_visible': 'test-only-command', 'reply': replies[0], 'reply_count': 1, 'click_coordinates': coordinates, 'unrelated_receipt_absent': True}
        session.key(KEY['esc'])
        ...
        current = 'inline-instruction-keeps-context'
        session.key(b'/')
        session.paste('working')
        session.wait(lambda: 'working' in session.text(), 'find managed task by actual title')
        session.key(b'\r')
        session.wait(lambda: 'Observed' in session.text() or 'OBSERVED' in session.text(), 'selected task work panel')
        click_text(session, 'Instruct')
        session.wait(lambda: 'INSTRUCTION TO' in session.text(), 'inline instruction composer')
        session.paste('Keep the recipient; literal $(text)')
        session.save(out / '08-inline-composer')
        interrupts_before=len([r for r in records(folder/'provider.jsonl') if r.get('method')=='turn/interrupt'])
        session.key(b'\x1bOQ')  # Hidden F2 stop must not execute while composing.
        assert len([r for r in records(folder/'provider.jsonl') if r.get('method')=='turn/interrupt'])==interrupts_before
        session.key(KEY['f7'])
        session.wait(lambda: 'Keep the recipient' in session.text(), 'expanded composer preserves draft')
        session.save(out / '09-expanded-composer')
        session.key(KEY['f7'])
        session.wait(lambda: 'Keep the recipient' in session.text(), 'collapsed composer preserves draft')
        click_text(session,'Send')
        session.wait(lambda:any(r.get('method')=='turn/steer' for r in records(folder/'provider.jsonl')), 'inline send steers existing managed task')
        steering=[r for r in records(folder/'provider.jsonl') if r.get('method')=='turn/steer']
        assert len(steering)==1 and steering[0]['params']['threadId']=='managed-2',steering
        assert steering[0]['params']['input']==[{'type':'text','text':'Keep the recipient; literal $(text)'}],steering
        assert len([r for r in records(folder/'provider.jsonl') if r.get('method')=='thread/start'])==2
        result[current]={'exact_thread':'managed-2','expand_collapse_preserves_draft':True,'hidden_stop_ignored':True,'steer_count':1,'no_duplicate_thread':True}
        session.key(KEY['esc'])
        session.key(KEY['esc'])
        ...
        if not any('Connections' in row for row in session.screen.display):
            click_text(session, 'More')
        ...
        session.wait(lambda: 'Connections / Sources' in session.text(), 'local source chooser')
        ...
        click_text(session, 'Back')
        session.wait(lambda: 'Connections / Sources' not in session.text(), 'source chooser returns to office')
        ...
        if 'Local sources' in session.text():
            session.key(b'3')
        session.wait(lambda: 'Connections / Sources' in session.text(), 'keyboard opens sources with mouse off')
        click_text(session, '[x] Codex')
