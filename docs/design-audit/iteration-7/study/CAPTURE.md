...
The preparation helpers were smoke-tested on macOS and reuse its Menlo font
...
2. Run `prepare_session.py P01` with Python including Pillow and the existing
   ...
   `docs/design-audit/tmp/iteration-7-workflows/study-P01` and prints a launcher
...
3. Invoke the printed launcher with `--condition tower`, `compact` or `reduced`.
...
```sh
python3 docs/design-audit/iteration-7/study/prepare_session.py P01
python3 docs/design-audit/iteration-7/study/prepare_session.py P01 --add-result
```
