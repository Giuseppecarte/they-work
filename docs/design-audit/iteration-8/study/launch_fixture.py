...
def main():
    ...
    parser.add_argument('--preflight', action='store_true', help='Engineering preparation only; no participant session')
    parser.add_argument('--candidate-manifest', type=Path, default=Path(__file__).with_name('baseline.json'))
    args = parser.parse_args()
    folder = args.fixture.resolve()
    parent = (ROOT / 'docs/design-audit/tmp/iteration-8-study').resolve()
    ...
    if not args.preflight and (not manifest.get('locked') or manifest.get('binary_sha256') != digest):
        parser.error('Freeze the baseline manifest before sessions; the executable SHA-256 must match. Use --preflight only for engineering preparation.')
    if args.preflight:
        print('ENGINEERING PREFLIGHT: no participant session or physical-terminal verdict.', flush=True)
    ...
    print('Synthetic fixture only. Binary SHA-256: ' + digest, flush=True)
